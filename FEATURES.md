...
| A13 | Environments CRUD — `/api/environments*` `server.ts:314-372` | Folder-of-accounts with a shared proxy. Renamed/deleted in the **Accounts** module (where they are created) | `create/update/deleteEnvironment` `AccountManager.ts:200` | `getTree` `:546`; `countInEnvironment` | `db.environments` | A10 (env proxy) |
| A13b | **Cascade delete** — `DELETE /api/environments/:id?cascade=1` | Delete an environment TOGETHER WITH its accounts (UI gates on a typed `DELETE`) | route tears each account down exactly as `DELETE /api/accounts/:username`, then `deleteEnvironment(id,{cascade})` | `removeMany` (one save); `AccountVault.removeAccounts` (one write); `pruneRuleTargets` drops rules left targeting nothing | `db.accounts/folders/environments/proxyRules`, `vault.enc`, inventory caches | A13, A14, proxy rules |
| A13c | Sign out of all devices — `POST /api/steam/:username/signout-all-devices` | Steam's account-wide session revocation (`twofactor/manage_action`, `action=deauthorize`) | `StoreService.deauthorizeAllDevices` | **409 for token-only accounts** (INV-A2: the refresh token is their only credential); on done/ambiguous drops the session + clears the stored token | Steam account state; `refresh_tokens`/vault | A2, sessions |
...
| E2 | Background price fill (per-exit budget) | Fetch missing/stale prices off request path | `ensureFilled` `:112`→`run` `:132` | `SteamPriceSource`/`CsFloatPriceSource` fetch; lanes round-robin across distinct exits, each exit capped at `LANES_PER_EXIT=3` and paced to ~0.86 req/s regardless of lane count | `prices.json` | E1,E5,E2b |
| E2b | Rotating-proxy detection | Let ONE rotating proxy use more than one exit's budget | `egressRotation.observe` (fired from `SessionManager.pricerIdentities`) | two ipify probes with `Connection: close`; different IPs ⇒ rotating ⇒ each session buckets as its own exit. Fail-safe: same-IP/non-200/transport-error/unmeasured all ⇒ static. Cached 30min, sync read, kill switch `SSIM_EGRESS_ROTATION_PROBE=0`. **⚠ never run against a real rotating provider** | none (RAM) | E2 |
