 ...
 * backlog is not counted as failed.
 */
...
export class LocalIpThrottle {
   ...
   * out its cooldown and then fetching).
   */
  run<T>(task: () => Promise<T>, opts?: { skip?: () => boolean }): Promise<T> {
    const scheduled = this.tail.then(async () => {
      ...
      // Re-check `skip` after the cooldown parked us. `refreshOne` reuses a
...
function sleep(ms: number): Promise<void> {
  // Unref the cooldown timer so a parked no-proxy task cannot, on its own,
