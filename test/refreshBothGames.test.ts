...
// ─────────────────────────────────────────────────────────────────────────────
//  Both-games bulk refresh (owner request): startRefresh fetches CS2 AND TF2 for
//  every account in ONE pass (same login/throttle slot), so the Steam wallet lands
//  in both caches with one timestamp and the TF2 tab never needs its own login
//  pass. A TF2-only failure surfaces as a "TF2:"-tagged partial failure WITHOUT
//  voiding the account's committed CS2 leg. Re-applied on the proxy-tank base: the
//  worker consults the per-proxy breaker BEFORE either leg, so a storming proxy is
//  deferred, not dialed. (Object.create exercises the shipped runRefresh without the
...
function makeSvc(refreshOneImpl: (u: string, g?: string) => Promise<unknown>): InventoryService & { job: { running: boolean; done: number; total: number; failed: Array<{ username: string; error: string }> } } {
  ...
  // Proxied accounts with NO proxy value → proxyKey null → breaker never blocks (tested separately).
  svc.accounts = { get: () => ({ network: { type: 'proxy' } }) };
