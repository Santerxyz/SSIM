...
### C14 — Update mis-placed/bricked via the unsigned `kind` flag — **MEDIUM · PROVEN (path) · ☑ FIXED (both halves)**
The Ed25519 signature covered `${latest}:${sha256}` only; `info.kind` (the swap-shape selector) was **outside** it. A MITM that cannot forge the signature could still flip `kind:'backend'→'single-exe'`; on a two-file sidecar with a sibling `SSIM.exe`, the swap then overwrote the GUI shell with the authentic backend artifact **and deleted the running backend** → a broken install. **Was breaking INV-G3.**

**Half 1 — brick (client-side, shipped, Phase 3):** the orphan-delete — the actual *harm* of a flipped `kind` — is gated on a **confirmed successful swap** (`buildSwapScript` `:swapped`/`:swapfail`). A failed `move` degrades to a no-op update. Test: `test/licenseUpdate.test.ts`.

**Half 2 — integrity (Phase 5): `kind` is now INSIDE the signed payload, so a flipped value is rejected.** Dual-signed during the transition so the existing fleet is never broken:
- **Server** (`ssim-license-server/src/signing.js` + `admin.js`): every release now emits BOTH `sig = Ed25519(${version}:${sha256})` (LEGACY, kept for the deployed fleet) AND `sigKind = Ed25519(${version}:${sha256}:${kind||'backend'})`. Both `/release/finalize` and the legacy `/version` paths. Tests: `ssim-license-server/test/signing.test.js` (old validates, new validates, **tampered kind FAILS**).
- **Client** (`Updater.ts`): the new client verifies **`sigKind`** (rejects a missing or tampered `kind`); it no longer uses `sig`. Old deployed clients keep using `sig` (still emitted). Test: `test/updateSignature.test.ts` + a recorded **cross-repo interop check** (server `releaseSignatures` → client `verifyUpdateSignature`: both kinds validate, tampered rejected).
- The canonical default for `kind` when absent is **`'backend'`** on BOTH sides (server `kind||'backend'` ≡ client `info.kind ?? 'backend'`).

**Rollout (the whole risk — see deploy plan): SERVER first (dual-sign live, verified in prod that BOTH an old client and the new client fetch+validate) → ONLY THEN publish the new client.** Never publish the client before the server is confirmed dual-signing.

> **DEFERRED CLEANUP (do NOT do now):** remove the legacy kind-less `sig` from the server signer **only after the entire fleet has migrated** to the kind-aware client. **Confirm migration before removing:** add a `ver` field to the client's `/heartbeat` body so the server can record each active seat's client version, then remove `sig` once 100% of seats seen within the offline-grace window report ≥ the kind-aware version (and no client logs `"refusing update – no sigKind"`). Until then, dropping `sig` would brick every old client's updater. Tracking crosses into the sibling `license-server` repo (see [[updater-fleet-dual-mode]]).
