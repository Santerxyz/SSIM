...
| # | Capability / behavior | Legacy ref | 💰 | Status |
|---|---|---|---|---|
| P-484 | Open Trade-Ups `openTradeUpModal(username)`: empty state "Scan this account for profitable trade-up contracts."; scope `· username` | app.js:6605–6613 | 💰 | ✅ done · empty-state already DS (`.empty`); scope span → t14 via shell; state writes/`observeOverlay`/`renderTuToolbar` call UNCHANGED (logic unchanged) |
| P-485 | Trade-up toolbar `renderTuToolbar`: Scan; when candidates → Select all / Clear / Execute (N) | app.js:6615–6629 | | ✅ done · toolbar already `.btn btn-primary/secondary/ghost/danger btn-sm` DS (no legacy markup); Scan/Select-all/Clear/Execute bindings byte-identical (logic unchanged) |
| P-486 | Scan trade-ups `tuScan`: POST `/api/tradeup/candidates {username}`; "Refreshing inventory & computing…"; auto-selects all; foot `N profitable contract(s) · M eligible input(s)` + warnings | app.js:6631–6647 | 💰 | ✅ done · `tuScan` byte-identical (POST `/api/tradeup/candidates` + auto-select-all + foot summary UNCHANGED); spinner/error states already DS (`.empty`) |
| P-487 | Trade-up candidate list `renderTuList`: per contract checkbox, rarity→outputRarity, collection pill, avg float (3dp), ~est-prices pill, no-asset-ids pill; inputs summary; outcomes (name/wear/prob %1dp/price); profit ±`fmtCents`, cost, EV | app.js:6649–6692 | 💰 | ✅ done · renderTuList re-skinned: rarity→output line t13 + arrow t10, inputs t10, outcome tokens `inline-flex…rounded-md bg-slate-950` → `.pill pill--neutral`, profit figure t16, cost/EV t10; `.surface` cards + status `.pill pill--warn/danger` kept; **money colors text-success/-danger + price-null dim kept = pricing honesty**; checkbox `data-tu-id` + `selCls` classList binding UNCHANGED |
| P-488 | Empty candidates: "No positive-profit trade-ups from this account's skins." | app.js:6652–6655 | | ✅ done · empty-candidates state already DS (`.empty`); unchanged |
| P-489 | Execute trade-ups `tuStart`: only executable (asset ids) chosen; `ssimConfirm` danger "Execute N trade-up(s)? …Each destroys 10 real items. IRREVERSIBLE. ⚠ GC not live-verified… start with 1…" → POST `/api/tradeup/execute`; poll | app.js:6694–6711 | 💰 | ✅ done · **`tuStart` EXECUTE handler byte-identical — ssimConfirm + api UNCHANGED** (danger IRREVERSIBLE gate + POST `/api/tradeup/execute` + GC-not-verified warning untouched) |
| P-490 | Trade-up exec poll (1.2s) `tuPollExec`: GET `/api/tradeup/execute-status`; line `Executing X/Y · submitted Z (C confirmed) · failed F [· cancelling]` or "Execution disabled (reason)"; Cancel btn; bounded error-retry → "status LOST, verify in-game" (never fabricates done) | app.js:6713–6747 | 💰 | ✅ done · `tuPollExec` byte-identical (foot progress markup + Cancel btn + bounded-retry "status LOST, verify in-game" honesty left intact) — poller NOT re-skinned, mirrors frozen `casketPollMove` (design-by-analogy note in CHANGES) |
| P-491 | Feature overlay shell `ensureFeatureOverlay(id,title,icon,width)`: generic modal-card w/ scope/toolbar/body/foot; observeOverlay; backdrop-click close | app.js:6576–6597 | | ✅ done · shell re-skinned: card `modal-card` → `bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl` (matches ban/buy/sell overlays), title t16, scope t14, foot t10; `data-scope/-toolbar/-body/-foot/-close` + `.modal-x` + widthClass param + `[id$="-overlay"]` + `observeOverlay` + close bindings UNCHANGED |
...
| # | Capability / behavior | Legacy ref | 💰 | Status |
|---|---|---|---|---|
| P-492 | Open Storage Units `openCasketModal(username)`: "Connecting to the game coordinator…"; unit `<select>`; GET `/api/casket/:u/list`; loads first unit's contents | app.js:6755–6772 | | ✅ done · `openCasketModal` toolbar template re-skinned (label t10, `<select>` t13) — template-string only; `/api/casket/:u/list` fetch + state writes + connecting spinner (`.empty`) UNCHANGED (logic unchanged) |
| P-493 | Unit select `renderCasketUnitSelect`: options `name (count/1000)` or "— no storage units —"; onchange reloads contents | app.js:6774–6782 | | ✅ done · `renderCasketUnitSelect` emits `<option>` strings only (no DS classes to swap); `onchange` reload handler byte-identical (logic unchanged) |
| P-494 | Load contents `loadCasketContents`: GET `/api/casket/:u/contents?casketId` | app.js:6784–6790 | | ✅ done · `loadCasketContents` byte-identical (GET contents; logic unchanged) |
| P-495 | Depositable inv rows `casketInvRows`: filters cache to in-inventory (not listed) w/ asset ids; search-filtered | app.js:6793–6798 | | ✅ done · `casketInvRows` pure logic, byte-identical (logic unchanged) |
| P-496 | Storable gate `casketStorable`: greys out Storage Unit / Collectible / Pass / Gift (GC-rejected) | app.js:6807–6813 | | ✅ done · `casketStorable` pure logic, byte-identical (logic unchanged) |
| P-497 | Two-panel render `renderCasketPanels`: Inventory panel (checkboxes, ×count, not-storable greyed) ↔ Unit panel (contents, capacity bar `count/1000`, def index); Deposit → / ← Withdraw; per-panel Select-all; filter input | app.js:6815–6872 | | ✅ done · `renderCasketPanels` re-skinned: item rows text-xs→t12, not-storable label text-3xs→t10, capacity-bar counter t10, panel-head counts text-2xs→t10, filter input t12; `.surface`/`.panel-head`/`.panel-title`/`.btn` already DS kept; every `data-ck-*` hook + all event bindings (search/inv/unit-item/select-all/deposit/withdraw) UNCHANGED |
| P-498 | Empty states: inv "No depositable items in cache." / unit "Empty storage unit." or error "…need the GC layer (install globaloffensive + SSIM_GC_VERIFIED=1)" | app.js:6833, 6840 | | ✅ done · empty states already DS (`.empty` + `.empty-icon text-warn`); GC-layer error triangle + sub-text honesty preserved |
| P-499 | Casket move (deposit/withdraw) `casketMove(direction)`: collects asset/item ids; `ssimConfirm` brand "Deposit/Withdraw N item(s)?"; POST `/api/casket/move {username,casketId,itemIds,direction}`; poll | app.js:6874–6894 | 💰 | ✅ done · **`casketMove` byte-identical — ssimConfirm + api UNCHANGED** (brand Deposit/Withdraw gate + POST `/api/casket/move` untouched) |
| P-500 | Casket move poll (1s / 2s err) `casketPollMove`: GET `/api/casket/move-status`; counters `direction: done/total · moved M [· unconfirmed U] · failed F`; error shown WITH partial counters (never error-only if progress); Cancel btn; budget-stop warn "N not attempted; run again"; cancelled prefix; unconfirmed → "verify in-game"; re-pulls cache + re-renders | app.js:6896–6949 | 💰 | ✅ done · `casketPollMove` byte-identical (counters + partial-progress-with-error honesty + Cancel + budget-stop/cancelled/unconfirmed toasts + post-move cache re-pull UNCHANGED); progress markup left intact (frozen handler) |
