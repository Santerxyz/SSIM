...
- [x] Phase 3 — re-skin view render functions V2–V19 in new_public/app.js (serial, idempotent via `_P3_DONE.md`, node --check gate per view). **DONE: V2 V3 V4 V5 V6 V7 V8💰 V9💰 V10💰 V11💰 V12💰 V13💰 V14💰 V15💰 V16💰 V17 V18 V19.** COMPLETE — all V2–V19 done.
- [x] Phase 4 — verification COMPLETE (VERIFICATION.md): global handler-safety proof (0 handler/API/state changes except documented game-toggle re-wire); 292/292 ids + all data-* hooks preserved; contract audit 101/101 endpoints byte-identical; 9/9 invariants PASS; 9 money-view adversarial verdicts all SAFE (0 regressions); PARITY 533/533 rows ticked; backend tsc 0 + 947 tests (0 fail). Live run → LIVE_TEST_CHECKLIST.md (owner's money-path testing).
- [x] Cutover — DONE (commit 89af69c): `public/` replaced with the new frontend (verified byte-identical to `new_public/`; legacy_public/ = rollback). Backend `tsc` exit 0; `npm test` unchanged-green (947/942-pass/0-fail — src/ byte-identical; no test asserts a version value). Entry-file/query-param contract preserved.
- [x] Version bump — DONE (commit ad7569a): 1.3.6 → **1.4.0** in package.json + src-tauri/tauri.conf.json.
- [~] Compile — **READY but blocked in THIS worktree environment.** `npm run build:tauri` fail-closes at `missing required secret LICENSE_PEPPER` (secrets.local.bat is gitignored + lives in the main checkout only), and the Tauri shell step needs `cargo`, which won't run here (no configured rustup default — the Rust toolchain is quarantined in this sandbox, RUSTUP_HOME→.claude-virus). Cannot resolve without touching the P4-owned main checkout or the quarantined toolchain (both out of bounds). **Owner step:** from a Rust-enabled checkout of `redesign/frontend-v2` with `secrets.local.bat` present + `rustup default stable` set, run `npm run build:tauri` → `release-tauri/SSIM/SSIM.exe` (v1.4.0). Do NOT publish (the 1.4.0 update-manifest publish is the owner's separate web-panel step). See OWNER_NOTES.md.
...
- 2026-07-07 · **V19 done → Phase 3 COMPLETE (all V2–V19).** Final view = settings/brand/residual-cleanup pass (no new modal — settings surfaces distributed across V16/V17/V18 + unlock/license portals, all re-verified already-DS). Wired `image-Photoroom.png` as app logo (invariant 8) across index/splash/unlock/license, retiring dead `logo.png` (kept in assets/, unreferenced — OWNER_NOTES). Re-skinned the 3 last legacy-Tailwind buttons (`#sda-conf-retry` + 2 CSFloat dashboard tabs → `.btn btn-secondary btn-sm`, class-only). node --check clean; div 250/250; 0 remaining logo.png refs. Next: Phase 4 verification.
